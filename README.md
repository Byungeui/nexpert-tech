...
**⚠ 분기를 추가할 때는 `BLOCK` 도 같이 늘어나야 한다.** 문단 분기는 "블록의 시작이
아닌 줄"만 먹는데, 그 판단을 `BLOCK` 하나가 한다. 분기만 늘리고 `BLOCK` 을 빼먹으면
문단이 먼저 삼켜 새 분기가 죽고, 반대로 `BLOCK` 에만 넣으면 아무도 그 줄을 소비하지
않아 **다시 무한 루프**다. 그래서 `BLOCK` 은 따로 적지 않고 분기들이 쓰는 정규식
(`HR`·`UL`·`OL`·`TABLE`·`FENCE`)에서 **조립한다** — 베껴 적는 순간 언젠가 어긋난다.

**⚠ 번호 목록의 자릿수는 두 자리로 묶여 있다**(`OL`). 풀면 `2026. 08. 21.` 같은
한국식 날짜가 목록 항목으로 둔갑한다 — 세무·견적 이야기를 하는 이 회사 답변에
실제로 나오는 형태다.
