...
**같은 이유로 `새 대화`·`저장된 대화` 버튼도 헤더에서 입력줄로 내렸다.** 처음에는
헤더 오른쪽 위에 뒀는데, **눈도 손도 입력창 근처에 있는데 반대편 구석까지 가야 했다.**
위의 `flex-end` 판단과 같은 이야기다. 지금은 입력칸 왼쪽에 붙어 있고, 좁은 화면에서는
CSS 가 글자를 감춰 아이콘만 남긴다(글자를 남기면 375px 에서 입력칸이 눌린다).

대화 목록은 **상설 사이드바가 아니라 서랍**이다. 위에서 2열을 되돌린 이유가 그대로
적용된다 — 목록을 늘 띄워 두면 그 열이 대화 내내 비어 있다. 여는 버튼이 입력줄 왼쪽에
있으므로 **서랍도 왼쪽에서 나온다.** 누른 곳과 열린 곳이 화면 양 끝으로 갈라지면
눈이 한 번 헤맨다.

⚠ 입력줄의 네 요소(보조 버튼 2 · 입력칸 · 전송)는 `min-height` 로 높이를 **명시해서**
맞춰 뒀다. 여백만 같게 두면 글꼴·줄높이 차이로 46/47/48px 처럼 어긋난다.
`flex-end` 라 바닥선은 맞지만 상자 높이가 달라 보인다.
...
| 무엇 | 어디에 | 파일 |
|---|---|---|
| 대화 본문 | **DynamoDB** `nexpert-tech-chats` (사본은 브라우저에도) | `src/chats.js` · `public/store.js` |
| 답변별 토큰 | 대화 항목 안에 | 〃 |
| 금액 | **저장하지 않는다** — 볼 때마다 요율로 계산 | `src/pricing.js` |
| 사용자별 사용량 | 서버 → CloudWatch 로그 | `server.js` |

### 처음에는 브라우저에만 뒀다 — 왜 옮겼는가

원래는 "중앙에 있어야 의미가 생기는 건 비용뿐이고 본문은 개인적이다"라고 보고
`localStorage` 만 썼다. 테이블·IAM·조회 API·삭제 권한이 한꺼번에 생기는 값을 치를
이유가 없다고 판단했고, 그 판단은 **혼자 한 기기에서 쓰는 동안에는 맞았다.**

바뀐 것은 두 가지다.

1. **폰과 PC가 이어지지 않는다.** `localStorage` 는 브라우저 프로필 단위다.
2. **iPhone Safari 는 한동안 방문이 없으면 이 저장소를 정리한다.** 불편이 아니라
   **기록을 잃는 문제**였다. 이게 결정타였다.

옮기는 작업이 가벼웠던 이유는 처음부터 `store.js` 를 `app.js` 밖으로 빼 뒀기 때문이다.
함수 이름과 반환 모양은 그대로 두고 안쪽만 `fetch` 로 바꿨다. 다만 **동기가 비동기로
바뀌는 것까지는 감출 수 없어서** 호출부에 `await` 가 붙었다.

### 브라우저 사본을 남긴 이유

서버 쓰기가 실패한 순간에도 방금 받은 답변은 화면에 있다. 사본이 없으면 그게 그대로
사라진다. 그래서 **쓸 때는 늘 양쪽에 쓰고**, 서버에 못 올린 대화의 id 를 `pending` 에
적어 뒀다가 다음 접속 때 올린다. 화면에도 "이 기기에만 저장됐습니다"라고 알린다.

⚠ **사본에 있는 걸 전부 올리지는 않는다. `pending` 에 적힌 것만 올린다.**
전부 올리면 A 브라우저에서 지운 대화가 B 브라우저의 낡은 사본에서 **되살아난다.**

### 남의 대화가 안 보이는 이유는 키 설계다

DynamoDB 파티션 키가 **검증된 이메일**이다. 조회는 늘 `user` 로 시작하므로 코드가
실수해도 다른 사람 항목이 결과에 섞일 수가 없다. `src/chats.js` 의 보안 설계는 이게 전부다.

⚠ **`chats.js` 에 넘기는 `user` 는 반드시 `access.requireUser()` 가 돌려준 값이어야 한다.**
검증 안 된 이메일을 넘기면 위 보장이 통째로 사라진다.

### 이제는 JWT 서명을 검증한다

예전에는 검증하지 않았다. 그때 이 값은 **사용량 로그의 이름표**일 뿐이라 틀려도 집계가
조금 어긋날 뿐이었기 때문이다. 대화가 서버에 있는 지금은 서버가 "이 사람의 대화를
돌려줘"를 판단하므로, **검증하지 않으면 헤더 한 줄로 남의 대화가 열린다.**

`src/access.js` 가 둘을 나눠 놓았다.

| 함수 | 검증 | 쓰는 곳 |
|---|---|---|
| `label(req)` | 안 함 | 로그 이름표 전용 |
| `requireUser(req)` | 서명·만료·audience | **대화 API 는 반드시 이것** |

⚠ **대화 API 에서 `label()` 을 쓰지 마라.** 그 순간 인증이 사라지고, 동작은 멀쩡해서
눈치채기 어렵다. 새 경로를 추가할 때는 `server.js` 의 `chatRoute()` 감싸개를 쓴다.

⚠ 알고리즘을 `RS256` 으로 못박아 두었다. 토큰 헤더의 `alg` 를 그대로 믿으면
`alg:none`·HS256 혼동 같은 고전적인 우회가 열린다.

⚠ **설정(`CF_ACCESS_TEAM`·`CF_ACCESS_AUD`)이 없으면 대화 저장이 꺼진다.** 무인증으로
열리지 않는다. 설정을 빠뜨린 배포는 기능이 하나 줄 뿐 사이트는 그대로 돌고, 화면은
브라우저 저장으로 돌아간다. **닫히는 쪽으로 실패하게 해 뒀다.**

⚠ `DEV_USER` 는 로컬 확인용이다. **`CF_ACCESS_AUD` 가 설정돼 있으면 무시된다** —
운영 태스크 정의에 실수로 들어가도 인증이 뚫리지 않게 하는 장치다.

### 고객사 이야기가 영구히 쌓이지 않게

테이블에 TTL 을 걸어 뒀다(`CHATS_TTL_DAYS`, 기본 90일). **마지막으로 손댄 시점 기준**이라
계속 쓰는 대화는 계속 남는다. 브라우저에만 있을 때는 없던 고민인데, 서버로 옮긴 값이다.

한 항목은 400KB 를 넘을 수 없다. 넘기 전에 오래된 메시지를 **질문·답변 한 쌍씩** 버린다
— 짝이 어긋나면 이력이 이상해진다.
...
이름표는 검증되면 이메일 그대로, 안 되면 `unverified:` 를 붙여 남긴다. 집계를 볼 때
확인된 신원인지 헷갈리지 않기 위해서다. ⚠ 여기서 검증에 실패해도 **답변은 계속한다** —
로그 이름표 때문에 질문이 막히면 안 된다. 대화 API 와 달리 권한 판단이 아니다.

⚠ 지금 하루 토큰 상한은 **전체 공용**이다(`limits.js`). 직원이 늘면 한 사람이 하루치를
다 쓸 수 있다. **검증이 붙었으므로 이제 1인당 상한을 걸 수 있다** — 다만 카운터가
프로세스 메모리라 재시작하면 0이 된다. 1인당으로 갈 때 그 저장 위치도 같이 정해야 한다.

### 비용 — 요율은 서버에만 있고, 금액은 저장하지 않는다

`src/pricing.js` 에 `us.anthropic.claude-opus-4-6-v1` 의 100만 토큰당 단가가 들어 있다
(2026-08-21 확인: 입력 $5 · 출력 $25 · 캐시 쓰기 $6.25 · 캐시 읽기 $0.50).
출처와 판단 근거는 그 파일 주석에 있다.

⚠ **모르는 요율은 비워 둔다. 추측해서 채우지 마라.** 값이 없으면 화면은 금액 없이
토큰 수만 보여준다. **틀린 금액은 없는 금액보다 나쁘다** — 그 숫자를 보고 "이 정도면
쓸 만한가"를 판단하기 때문이다. 모델을 바꾸면 그 모델 항목을 **확인해서** 추가한다.
표에 없는 모델은 옆 값을 빌려 쓰지 않고 자동으로 금액 없이 나온다.

⚠ **금액은 저장하지 않는다.** 화면은 토큰만 보관하고, 표시할 때마다 서버가 준
요율(`/api/categories` 의 `rates`)로 다시 계산한다. 답변마다 금액을 굳혀 두면 요율을
고쳐도 지난 대화는 옛 금액을 들고 있어 **한 화면에 두 기준이 섞인다.** 토큰은 사실이라
변하지 않지만 요율은 바뀐다 — 변하는 쪽을 저장하지 않는 게 맞다.
(반대로 **CloudWatch 로그에는 그때의 금액을 남긴다.** 그건 회계 기록이라 당시 요율로
굳는 게 맞다.)

⚠ **이 금액은 참고용 추정이고 정본은 AWS 청구서다.** 월말에 Cost Explorer 의 Bedrock
비용과 앱 합계를 한 번 대조해 보고, 어긋나면 `src/pricing.js` 를 고친다.

⚠ `thinking: adaptive` 를 쓰므로 **화면에 안 보이는 사고 토큰도 출력으로 과금된다.**
`output_tokens` 에 이미 포함돼 있다 — 답변 길이에 비해 출력이 많아 보이는 이유가 이것이다.

캐시 쓰기가 입력보다 비싸서 **첫 질문이 이어지는 질문보다 비싸다**(실측 $0.033 대 $0.005).
금액이 들쭉날쭉해 보이면 사용량 줄에 마우스를 올려 내역을 본다.
...
| `PRICE_INPUT_PER_1M` | | `src/pricing.js` 값 | 100만 입력 토큰당 USD. 요율이 바뀌었을 때 **배포 없이** 새 태스크 정의만으로 고치는 용도 |
| `PRICE_OUTPUT_PER_1M` | | 〃 | 100만 출력 토큰당 USD |
| `PRICE_CACHE_WRITE_PER_1M` | | 〃 | 100만 캐시 **쓰기** 토큰당 USD. 입력과 요율이 다르다 |
| `PRICE_CACHE_READ_PER_1M` | | 〃 | 100만 캐시 **읽기** 토큰당 USD |
| `CF_ACCESS_TEAM` | ★ | (없음) | Cloudflare Zero Trust 팀 이름. `https://<이것>.cloudflareaccess.com` |
| `CF_ACCESS_AUD` | ★ | (없음) | Access 애플리케이션의 **Application Audience (AUD) Tag** |
| `CHATS_TABLE` | | `nexpert-tech-chats` | DynamoDB 테이블 |
| `CHATS_REGION` | | `ap-northeast-2` | ⚠ Bedrock 과 달리 **ECS 와 같은 리전**이다 |
| `CHATS_TTL_DAYS` | | `90` | 대화 자동 삭제까지의 일수 (마지막 수정 기준) |
| `DEV_USER` | | (없음) | **로컬 전용.** Access 없이 사용자를 흉내낸다. `CF_ACCESS_AUD` 가 있으면 무시된다 |

★ 둘 다 있어야 대화 저장이 켜진다. 없으면 기능이 꺼질 뿐 사이트는 그대로 돈다 —
**무인증으로 열리는 것보다 닫히는 편이 안전하기 때문이다.**
